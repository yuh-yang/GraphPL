...
class ET_duo_emb(BaseModel):
    def __init__(self, dataset, pretrained_model=None, phase='pretrain'):
        ...
        if self.phase == 'pretrain' or self.phase == 'vanilla' or self.phase == 'for_tune':
            ...
            if self.phase == 'for_tune' and args.ab != 'no_gating':
                self.emb_gate = self.random_gate
            else:
                self.emb_gate = lambda x: x
        
        elif self.phase == "finetune":
            pretrained_model, tuned_model = pretrained_model
            pre_user_emb, pre_item_emb = pretrained_model.generate()
            tuned_user_emb, tuned_item_emb = tuned_model.generate()
            ...
            self.tuned_user_emb = nn.Parameter(tuned_user_emb).requires_grad_(True)
            self.tuned_item_emb = nn.Parameter(tuned_item_emb).requires_grad_(True)
            ...
            self.user_embedding = nn.Parameter(init(torch.empty(self.num_users, self.emb_size))).requires_grad_(True)
            self.item_embedding = nn.Parameter(init(torch.empty(self.num_items, self.emb_size))).requires_grad_(True)

            self.tuned_user_emb = nn.Parameter(init(torch.empty(self.num_users, self.emb_size))).requires_grad_(True)
            self.tuned_item_emb = nn.Parameter(init(torch.empty(self.num_items, self.emb_size))).requires_grad_(True)
    ...
    def simgcl_noise(self, x):
        random_noise = torch.rand_like(x).to(x.device)
        return x + torch.sign(x) * F.normalize(random_noise, dim=-1) * 0.1

    def random_gate(self, x):
        # 复现random gate
        # gating_weight = init(torch.empty(args.emb_size, args.emb_size)).to(args.device)
        # gating_bias = init(torch.empty(1, args.emb_size)).to(args.device)
    ...
    def forward(self, edges, edge_norm, edge_times, max_time_step=None):
        ...
        if self.phase != "for_tune":
            all_emb_last_tuned = torch.cat([self.tuned_user_emb, self.tuned_item_emb], dim=0)
            all_emb = 2/3 * all_emb + 1/3 * all_emb_last_tuned
