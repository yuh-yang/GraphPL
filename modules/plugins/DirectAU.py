...
from modules.plugins.BasePluginModel import BasePluginModel
...
class DirectAU(BasePluginModel):
    ...
    def cal_loss(self, batch_data):
        ...
        align_loss = self.alignment(batch_user_emb, pos_item_emb)
        uni_loss = self.gamma * (self.uniformity(batch_user_emb) + self.uniformity(pos_item_emb)) / 2

        loss = align_loss + uni_loss

        loss_dict = {
            "loss": loss.item(),
            "align_loss": align_loss.item(),
            "uni_loss": uni_loss.item(),
        }
