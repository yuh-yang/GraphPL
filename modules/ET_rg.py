...
class ET_rg(BaseModel):
    def __init__(self, dataset, pretrained_model=None, phase='pretrain'):
        ...
        if self.phase == 'pretrain' or self.phase == 'vanilla' or self.phase == 'for_tune':
            ...
            if self.phase == 'for_tune' and args.ab != 'no_gating':
                ...
                # self.emb_gate = self.simgcl_noise
            else:
            ...
            # add guassian noise
            # pre_user_emb += torch.randn_like(pre_user_emb) * 0.01
            # pre_item_emb += torch.randn_like(pre_item_emb) * 0.01

            # add simgcl_like noise
            # pre_user_emb += torch.sign(pre_user_emb) * F.normalize(torch.randn_like(pre_user_emb)) * 0.1
            # pre_item_emb += torch.sign(pre_item_emb) * F.normalize(torch.randn_like(pre_item_emb)) * 0.1
    ...
    def forward(self, edges, edge_norm, edge_times, max_time_step=None):
        ...
        # edge_norm = scatter_softmax(edge_norm + time_norm, edges[:, 1], dim_size=self.num_users+self.num_items)
