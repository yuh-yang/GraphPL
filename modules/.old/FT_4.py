...
def cal_uniformity(emb, temperature=1.0):
    emb = F.normalize(emb, dim=1)
    neg_score = emb.mm(emb.t())
    neg_score = torch.exp(neg_score / temperature).sum(1)
    pos_score = 1 / temperature
    cl_loss = -torch.log(pos_score / neg_score+10e-6)
    return torch.mean(cl_loss)

class FT_4(BaseModel):
    ...
    def cal_loss(self, batch_data):
        ...
        ## variance loss, emplify the variance of embeddings
        # var_loss = args.var_coef * (cal_uniformity(user_emb_ssl[users]) + cal_uniformity(item_emb_ssl[pos_items]))

        var_loss = args.var_coef * (cal_uniformity(torch.cat([user_emb_ssl[users], item_emb_ssl[pos_items]], dim=0)))
