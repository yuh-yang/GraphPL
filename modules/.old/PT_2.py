...
class PT_2(BaseModel):
    def __init__(self, dataset, pretrained_model=None, phase='pretrain'):
        ...
        if self.phase == 'pretrain' or self.phase == 'vanilla':
            ...
            self.self_gate = lambda x: torch.mul(x, torch.sigmoid(torch.matmul(x, self.gating_weight) + self.gating_bias))

            self.prompt_emb = nn.Parameter(init(torch.empty(self.num_users + self.num_items, 16)))
            self.prompt_mlp = nn.Sequential(
                nn.Linear(16, 32),
                nn.ReLU(),
                nn.Linear(32, args.emb_size),
            )
            self.prompt_gating_weight = nn.Parameter(init(torch.empty(2*args.emb_size, 1)))
    ...
    def _add_prompt(self, emb):
        prompt = self.prompt_mlp(self.prompt_emb)
        prompt_gate = torch.sigmoid(torch.matmul(torch.cat([emb, prompt], dim=1), self.prompt_gating_weight))
        emb = self.self_gate(emb)
        return emb + (1-prompt_gate) / prompt_gate * prompt
