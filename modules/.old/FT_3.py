...
class FT_3(BaseModel):
    def __init__(self, dataset, pretrained_model=None, phase='pretrain'):
        ...
        if self.phase == 'pretrain' or self.phase == 'vanilla':
            ...
            self.left_emb_mlp = nn.Sequential(
                nn.Linear(args.emb_size, 32),
                nn.ReLU(),
                nn.Linear(32, 16),
            )
            self.right_emb_mlp = nn.Sequential(
                nn.Linear(16, 32),
                nn.ReLU(),
                nn.Linear(32, args.emb_size),
            )
            # self.new_emb_gating_weight = nn.Parameter(init(torch.empty(2*args.emb_size, 1)))
            self.new_emb_gating_weight = nn.Parameter(init(torch.empty(args.emb_size, args.emb_size)))
            self.new_emb_gating_bias = nn.Parameter(init(torch.empty(1, args.emb_size)))
    ...
    def _channel_gate(self, emb_1, emb_2):
        ...
        gate = torch.sigmoid(torch.matmul(emb_2, self.new_emb_gating_weight) + self.new_emb_gating_bias)
        return emb_1 + torch.mul(emb_2, gate)
    ...
    def cal_loss(self, batch_data):
        ...
        new_emb = self.left_emb_mlp(torch.cat([self.pre_user_emb, self.pre_item_emb], dim=0))
        user_emb_new, item_emb_new = self.forward(edges, edge_norm, all_emb=new_emb)
        batch_user_emb_new = self.right_emb_mlp(user_emb_new[users])
        pos_item_emb_new = self.right_emb_mlp(item_emb_new[pos_items])
        neg_item_emb_new = self.right_emb_mlp(item_emb_new[neg_items])
